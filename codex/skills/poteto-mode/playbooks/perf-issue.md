...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model. If none is configured, omit the model field so the current host's model is used. Review the diff. Capture a post-fix trace.
