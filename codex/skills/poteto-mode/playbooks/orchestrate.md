...
- **Worker / verifier.** Always local — Codex has no cloud-agent equivalent, so keep the in-flight count small (two or three, not ten). Workers necessarily need this machine for: local runtime verification of a UI or CLI; reading local transcripts under `~/.codex/sessions/`; simulators and local IDE state; auth that exists only here. Workers still get self-contained briefs: inline what they need or point at repo paths, so a brief never depends on the coordinator's context. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model from its worker.
