...
5. **Spawn one blinded judge** on a different model per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript in `~/.codex/sessions/` (filed by date, so match each file's first-line `payload.cwd` to this workspace) (the system prompt names this path). Codex files sessions by date, not by workspace, so filter each rollout by its first-line `payload.cwd`; reading unfiltered crosses workspace boundaries into private chats from unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
