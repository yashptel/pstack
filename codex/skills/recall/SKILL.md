...
Transcripts live at `~/.codex/sessions/<YYYY>/<MM>/<DD>/rollout-<timestamp>-<uuid>.jsonl`, partitioned by **date, not by project**. The first line of each file is a session-meta record whose `payload.cwd` names the workspace that session ran in — that field is the only way to tell one project's sessions from another's. `~/.codex/session_index.jsonl` indexes them by `id`, `thread_name`, and `updated_at`. Every other line is one chat message.
