---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill, then runs it through unslop, optionally pulling fresh evidence from recent transcripts."
...
This skill orchestrates three others: an inline mining pass (see step 1), skill authoring, and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with a question to the user (unless they already said "update my skill" or similar):
...
Locate this workspace's transcripts before fanning out. Codex files sessions by date at `~/.codex/sessions/<YYYY>/<MM>/<DD>/`, **not by workspace**, so no path fences off this project. Filter by content: read the first line of each rollout file and keep only those whose `payload.cwd` is this workspace. Skipping that check reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the a question to the user tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.
...
- Frontmatter formatting: keep the YAML valid. Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Apply the **unslop** skill to every line. Both apply to any agent-read prose, not just skills.
...
- User wants a task-specific skill (not working conventions): author it directly, no mining required.
