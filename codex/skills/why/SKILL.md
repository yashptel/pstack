...
Before spawning investigators, list the MCP servers available in this session. Connected MCP tools appear in your own tool list, and `codex mcp list` names the configured servers if you need the list confirmed.
...
- `model`: your configured why-investigators model (default `gpt-5.4-mini`)
...
- subagent: the default subagent
- `model`: your configured why-synthesizer model (default `gpt-5.5`)
