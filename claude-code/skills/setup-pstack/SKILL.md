...
Write `~/.claude/pstack-models.md`, a plain markdown file that sets pstack's model per role. The skills read it and fall back to their inline defaults when a line is absent, so this is an override layer, not a requirement.

## Steps

### 1. Detect available models

Enumerate the model slugs you can pass to an Agent-tool subagent in this session; that is the dependable source. If the host exposes a models API or CLI that lists your entitled models, prefer it for completeness. If you cannot detect any, ask the user to paste the slugs they have access to. Never write a real slug you have not confirmed is available. The aliases `inherit-parent` and `auto` are always valid even though they are not detected slugs.
...
how explorer: sonnet
how explainer: fable
how critics: fable, opus, sonnet, opus
