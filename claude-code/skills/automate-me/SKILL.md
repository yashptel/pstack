...
Locate the active workspace's transcripts before fanning out. They live under `~/.claude/projects/<slug>/`, where `<slug>` is the workspace path with each "/" turned into "-". Use only that directory. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
