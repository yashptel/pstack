...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the remote agent's status in your host's UI. Transcript mtime is not liveness.
