...
Transcripts live at `~/.claude/projects/<slug>/<session-id>.jsonl`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-", then prefixed with "-" (so `/Users/you/proj` becomes `-Users-you-proj`). Subagent transcripts sit under `~/.claude/projects/<slug>/<session-id>/subagents/*.jsonl`. Every line is one chat message.
