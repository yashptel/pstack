...
Before spawning investigators, list the MCP servers available in this session. Connected MCP tools appear in your own tool list namespaced as `mcp__<server>__<tool>`, so read the tools you have rather than a config file; `/mcp` shows the connected servers if you need the list confirmed.
