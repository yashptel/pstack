...
- `model`: your configured how-explorer model; if none is configured, omit the field so the current host's model is used
- `readonly`: `true`
...
- `subagent_type`: the general-purpose agent
- `model`: your configured how-explainer model; if none is configured, omit the field so the current host's model is used
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list. If no list is configured, omit the model field for each critic so each uses the current host's model. Spawn them all in a single message.
