...
You don't describe your style, because [`/automate-me`](../../claude-code/skills/automate-me/SKILL.md) reads it out of your history. It mines your recent transcripts for the active workspace for repeated preferences, in how you like replies, delegation, verification, code, prose, and process, then asks you which patterns are really you. It drafts `.claude/skills/<your-name>-mode/SKILL.md` (or `.agents/skills/...` on Codex) through the skill-authoring loop flow, runs the draft through [`/unslop`](../../claude-code/skills/unslop/SKILL.md), and opens a PR from a worktree so you review it like any other change.
...
Writing a skill matches the [Authoring or modifying a skill playbook](../../claude-code/skills/poteto-mode/playbooks/authoring-a-skill.md), which routes through the skill-authoring loop, validates the frontmatter and links, and ships the result through the Opening a PR playbook. Agent-facing prose has a higher bar than human prose, because an unhelpful sentence becomes an instruction some future agent follows. Let the playbook hold that bar rather than writing a `SKILL.md` freehand.

One special case has its own generator. A skill that must drive your app and prove behavior is a verification skill, so use [`/create-verification-skill`](../../claude-code/skills/create-verification-skill/SKILL.md) and [`/maintain-verification-skill`](../../claude-code/skills/maintain-verification-skill/SKILL.md) instead. [Verify and ship](./06-verify-and-ship.md#create-a-project-verification-skill) covers both.
