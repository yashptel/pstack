...
Locate this workspace's transcripts before fanning out. The host may file sessions by date rather than workspace, so no path necessarily fences off this project. Use the host's native transcript search or inspect each session's workspace metadata and keep only this workspace. Skipping that check reads private chats from unrelated projects.
