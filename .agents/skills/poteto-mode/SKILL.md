---
name: poteto-mode
description: poteto's agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Use for poteto or requests to work in this style.
disable-model-invocation: true
---

> **When to reach for this.** New task? Playbook match or rigor needed -> apply poteto-mode. Casual turn or user opts out -> don't.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the skill-authoring loop.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill.
- Before commit → strip AI slop from the diff yourself: dead abstractions, defensive noise, restated comments, and style that does not match the surrounding code.
- Before review → the **no-comments** skill.
- Shipping UI / IDE / CLI → verify on the real surface with a repeatable harness before you claim it works. Reuse the repo's own Playwright, browser, Electron, or CLI test harness if it has one; otherwise stand up a temporary local harness around the dev server, debug port, or a scripted terminal session. Capture evidence — screenshots, accessibility snapshots, transcripts, timings — and verify against the artifact, never a self-report. For bug fixes, reproduce on that same surface yourself first; hand to the user only under the narrow Bug fix step 1 exception.
...
**Use the host's native subagent mechanism for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own subagent role for diverse review; respect what the skill prescribes, don't override it.

**Defaults for every subagent delegation.** Use the host's native background, agent, read-only, and file-reference options when available. Use the configured model for the role; when no role model is configured, use the current host's model. Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to the host's strongest judgment model when the task needs judgment or the intent is vague, and to its strongest instruction-following model when the work is a precisely specified sequence of steps to execute to the letter. Trivial mechanical edits go to the host's fastest suitable model. Role settings from the host's native configuration override these defaults, and a role value of `inherit-parent` or `auto` runs that role on the parent chat model.
