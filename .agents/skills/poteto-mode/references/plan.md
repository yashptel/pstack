...
- Prefer the host's native subagent mechanism. Use its default subagent when no specialized role is configured.
- Use the configured model per role. If no role model is configured, use the current host's model.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the skill-authoring loop.
...
- Browser / Electron / Web UIs, CLIs and TUIs: drive the real surface with a repeatable harness, preferring the repo's own, and verify against captured evidence.
...
- strip AI slop from the diff over each diff before commit. the **unslop** skill over any prose surface.
