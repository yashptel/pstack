...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the configured bug-fix model, or the current host's model when none is configured, with a specific scope; review the diff.
