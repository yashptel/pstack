...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the configured perf-issue model, or the current host's model when none is configured; review the diff. Capture a post-fix trace.
