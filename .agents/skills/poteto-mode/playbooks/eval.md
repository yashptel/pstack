...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript through the host's native session store and filter by this workspace's metadata. Sessions may be filed by date rather than workspace, so reading unfiltered crosses workspace boundaries into private chats from unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
