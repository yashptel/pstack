...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use the host's native file and search tools as needed. The explorers did the heavy lifting, so you shouldn't need to re-explore from scratch.
