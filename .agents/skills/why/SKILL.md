...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, current location, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the MCP servers available in this session. Connected MCP tools appear in your own tool list; use the host's native MCP listing interface if you need the list confirmed.
...
Use the host's native subagent mechanism for each investigator. Use the configured why-investigators model, or the current host's model when none is configured. Give investigators the host mode that preserves MCP access. Investigators still should not write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent through the host's native mechanism. Use the configured why-synthesizer model, or the current host's model when none is configured. Give it the host mode that preserves MCP access. The synthesizer's quality check spot-verifies citations, which can require MCP access.
