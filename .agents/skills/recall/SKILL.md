...
Transcripts live in the host's native session store, which may be partitioned by **date, not by project**. Use the host's workspace metadata to tell one project's sessions from another's. Every other line is one chat message.
